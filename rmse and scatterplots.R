...
simFile <- 'simulated'              #MVNHMM simulation output
copulaSimFile <- 'simulated'              #File generated from gammacorrels.R
obsFile <- 'original'                 #Observation data in MVNHMM format
date <- rep(seq(ymd('2001-01-01'), ymd('2001-03-31'), by = 'days'), 20)
month <- month(date)
day <- day(date)
year <- rep(2001:2020, each = 90)
date <- make_date(year, month, day)
date <- substr(date, 1, 7)
head(date)

obsdata <- read.table(obsFile,header = F)
hmmoutput <- read.table(simFile,header = F)
copulaoutput <- read.table(copulaSimFile, header = F)

month <- rep(c(1,2,3),20)
...
summaryplot <- data.frame(month = rep(month.abb[7:9], each = 1000), mean.obs = tmp1, mean.sim = tmp2, prop.obs = tmp3, prop.sim = tmp4)
...
ggplot(summaryplot,aes(x = mean.obs, y = mean.sim)) + geom_point() + geom_abline() +
      ...
      labs(x = 'Precipitation based on true parameters (mm)', y = 'Precipitation based on estimated parameters (mm)', col = 'Month') +
      annotate(geom = "text", x = xrng[1], y = yrng[2], label = 'RMSE = 11.48 mm', hjust = 0, vjust = 1, size = 7)
...
ggplot(summaryplot, aes(x = prop.obs,y = prop.sim)) + geom_point() + geom_abline() +
      theme(panel.grid.minor = element_blank(), plot.title = element_text(hjust = 0.5, size = 15), 
      axis.text.x = element_text(color = "grey20", size = 12, angle = 0, hjust = .5, vjust = .5, face = "plain"),
      axis.text.y = element_text(color = "grey20", size = 12, angle = 0, hjust = 1, vjust = 0, face = "plain"),  
      axis.title.x = element_text(color = "grey20", size = 17, angle = 0, hjust = .5, vjust = 0, face = "plain"),
      ...
      legend.text = element_text(size=12), legend.title = element_text(size=15)) +
      guides(colour = guide_legend(override.aes = list(size=3))) +
      labs(x = 'Proportion of dry days based on true parameters', y = 'Proportion of dry days based on estimated parameters', col = 'Month') +
